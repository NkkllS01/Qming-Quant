...
Qiming Quant is organized as a personal OKX futures trading system with a CLI-first control surface, local state storage, and strict live-trading gates.

```mermaid
flowchart TB
    operator["Operator CLI"]
    config["Settings and runtime log"]
    app["app.main command router"]

    okxGateway["OKX gateway"]
    okxRest["OKX REST API"]
    okxWs["OKX WebSocket API"]

    marketData["market_data candle sync"]
    storage["storage repositories"]
    strategy["strategies and runner"]
    backtest["backtest engine"]
    simulation["simulation engine"]
    risk["portfolio risk and symbol lease"]
    execution["execution order factory and managers"]
    live["live state, reconciliation, trading gate"]
...
The normal development and validation flow is data-first. Real order placement stays behind local data checks, risk checks, reconciliation, and operator emergency controls.

```mermaid
flowchart LR
    candles["OKX candles and instruments"]
    localData["local normalized data"]
    dataGate["data gate and gap checks"]
    strategy["strategy signal"]
    risk["portfolio risk decision"]
    orderIntent["order intent"]
    sim["simulation or backtest"]
    liveCheck["live order dry-run"]
    liveGate["trading gate"]
    okxOrder["OKX order adapter"]
    ...
    risk --> orderIntent
    orderIntent --> sim
    orderIntent --> liveCheck
    liveCheck --> liveGate
    liveGate --> okxOrder
```

Live trading is intentionally fail-closed. A live order must pass local policy, fresh market data checks, equity risk checks, emergency pause state, and REST reconciliation before the internal OKX order adapter can be used.

```mermaid
flowchart TB
    intent["OrderIntent"]
    policy["LiveOrderPolicy"]
    instrument["instrument spec check"]
    duplicate["duplicate client order id check"]
    pause["manual emergency pause"]
    equity["equity risk guard"]
    market["fresh mark price guard"]
    reconcile["REST reconciliation"]
    ...
    instrument -. reject .-> blocked
    duplicate -. reject .-> blocked
    pause -. paused .-> blocked
    equity -. breach .-> blocked
    market -. stale or missing .-> blocked
    reconcile -. mismatch .-> blocked
```

The trading ownership model keeps account, bot, strategy, and symbol lineage attached to every signal, order intent, order, fill, and live-state update.

```mermaid
flowchart TB
    account["Account: OKX subaccount"]
    bot["Bot: one isolated runtime"]
    strategyA["StrategyInstance: BTC trend"]
    strategyB["StrategyInstance: ETH crossover"]
    btc["Instrument: BTC-USDT-SWAP"]
    eth["Instrument: ETH-USDT-SWAP"]
    signal["Signal"]
    riskDecision["RiskDecision"]
    orderIntent["OrderIntent"]
    fill["Fill and Position"]
